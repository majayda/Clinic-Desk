...
<div class="card">
  ...
  <div class="card-body">
    <form class="form-inline mb-3">
      ...
      <input class="form-control mr-2" name="search" value="<?= e($search) ?>" placeholder="Search">
      <select class="form-control mr-2" name="role">
        <option value="">All roles</option>
        <?php foreach (['admin','doctor','patient'] as $r): ?><option value="<?= $r ?>" <?= $role === $r ? 'selected' : '' ?>><?= ucfirst($r) ?></option><?php endforeach; ?>
      </select>
    ...
    <?php foreach ($users as $row): ?>
      <tr>
        ...
        <td class="actions">
          <a class="btn btn-sm btn-info" href="<?= e(url('users', 'edit', ['id' => $row['id']])) ?>">Edit</a>
          <?php if ((int) $row['id'] !== Auth::id()): ?>
            <form method="post" action="<?= e(url('users', 'toggleActive')) ?>">
              <input type="hidden" name="csrf_token" value="<?= e(CSRF::generateToken()) ?>">
              <input type="hidden" name="id" value="<?= (int) $row['id'] ?>">
              <button class="btn btn-sm btn-warning"><?= (int) $row['is_active'] ? 'Deactivate' : 'Activate' ?></button>
            </form>
            <form method="post" action="<?= e(url('users', 'delete')) ?>" onsubmit="return confirm('Delete this user?')">
              <input type="hidden" name="csrf_token" value="<?= e(CSRF::generateToken()) ?>">
              <input type="hidden" name="id" value="<?= (int) $row['id'] ?>">
